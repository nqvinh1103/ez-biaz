...
function AccountInfo() {
  ...
  const planEndsAt = plan?.endsAt ? new Date(plan.endsAt) : null;
  const planStartsAt = plan?.startsAt ? new Date(plan.startsAt) : null;
  const planActive = !!plan && (!planEndsAt || planEndsAt.getTime() > Date.now());
      ...
      <div className="rounded-2xl border border-[#e6e6e6] bg-white px-6 py-5">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <svg className="h-4 w-4 text-[#ad93e6]" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
            <h2 className="text-sm font-bold text-[#121212]">Subscription</h2>
          </div>
          ...
          {planActive ? (
            ...
            <span className="rounded-full bg-[#fafafa] border border-[#e6e6e6] px-2 py-0.5 text-[10px] font-semibold text-[#737373]">
              No active plan
            </span>
          )}
        </div>
        ...
        {planActive ? (
          <div className="rounded-xl border border-[#e6e6e6] bg-[#fafafa] px-4">
            <InfoRow label="Plan" value={plan?.name ?? plan?.planId} />
            <InfoRow label="Plan ID" value={plan?.planId} />
            <InfoRow label="Price" value={plan?.price != null ? `${plan.price} VNĐ` : "—"} />
            <InfoRow label="Starts" value={planStartsAt ? planStartsAt.toLocaleString() : plan?.startsAt} />
            <InfoRow label="Ends" value={planEndsAt ? planEndsAt.toLocaleString() : plan?.endsAt} />
          </div>
        ) : (
          <div className="flex items-center justify-between gap-4 rounded-xl border border-[#e6e6e6] bg-[#fafafa] px-4 py-4">
            <p className="text-sm text-[#737373]">You don't have an active subscription.</p>
            <Link
              to="/subscription"
              className="inline-flex items-center justify-center rounded-full bg-[#ad93e6] px-4 py-2 text-xs font-semibold text-white hover:bg-[#9d7ed9] transition-colors"
            >
              Upgrade
            </Link>
          </div>
