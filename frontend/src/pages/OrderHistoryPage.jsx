...
/* ── Order Card ────────────────────────────────────────────────────────── */
function OrderCard({ order, onReceived, isRated, onRate }) {
  ...
  return (
    <li className="rounded-2xl border border-[#e6e6e6] bg-white overflow-hidden shadow-sm">
      ...
      <div className="flex flex-col gap-3 p-4">
        ...
        {/* Rating state for delivered orders */}
        {order.status === "delivered" && (
          isRated ? (
            <div className="mt-1 flex items-center gap-2 rounded-lg border border-[#f0f0f0] bg-[#fafafa] px-3 py-2">
              <StarDisplay rating={order._localRating ?? 5} size="sm" />
              <span className="text-xs text-[#737373]">Reviewed</span>
              <span className="ml-auto text-[10px] font-semibold text-[#22c55e]">✓</span>
            </div>
          ) : (
            <button
              onClick={() => onRate(order)}
              className="mt-1 w-full rounded-lg border border-[#ad93e6] py-2 text-sm font-semibold text-[#5b3f9e] transition-colors hover:bg-[rgba(173,147,230,0.08)] flex items-center justify-center gap-1.5"
            >
              <svg className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
...
export default function OrderHistoryPage() {
  ...
  const [ratedOrders, setRatedOrders] = useState(new Set()); // orderId set
  ...
  const handleReceived = async (orderId) => {
    ...
    if (res.success) {
      ...
      // auto-open rating modal after mark as received
      const order = orders.find((o) => o.id === orderId);
      if (order) setRatingOrder({ ...order, status: "delivered" });
    }
  };

  const handleRated = (orderId) => {
